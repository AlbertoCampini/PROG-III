...
public class MainClient extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        // CSS
        scene.getStylesheets().add( Objects.requireNonNull(getClass().getClassLoader().getResource("glisten.css")).toExternalForm() );
        scene.getStylesheets().add( Objects.requireNonNull(getClass().getClassLoader().getResource("client.css")).toExternalForm() );

        root.setCenter(loader.load());
        UIController controller = loader.getController();    // Obtaining UI Controller
        controller.initModel(uiMail, uiMailList, mailLists); // Model initialization
