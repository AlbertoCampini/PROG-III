...
public class RequestHandler implements Runnable {
    ...
    // Modifica i flag SEEN, FAVOURITE o elimina la mail
    private Response manageAlterMailReference(Request request, boolean value, String action, int type) {
        ...
        if (data != null && data.getClass() == Integer.class) {
            ...
            if (currentMailReference != null) {
                ...
                /* Decrease the linkCounter value */
                if (action.equals("Delete")) {

                    if (!currentMailReference.getMail().equals(request.getUser())) {    // If the user that has sent the mail isn't us
                        locksManager.getUserWithLocks(currentMailReference.getMail()).lockWrite();          // Get user's lock
                    }
                    Mail mail = MailReferenceFileManager.getMail(currentMailReference, locksManager);       // Get the mail
                    if (mail != null) {
                        mail.setLinkCount(mail.getLinkCount() - 1);                                         // Decrease the linkCounter value
                    ...
                    if (!currentMailReference.getMail().equals(request.getUser())) {    // If the user that has sent the mail isn't us
                        locksManager.getUserWithLocks(currentMailReference.getMail()).releaseWrite();       // Release the lock
                    }
                    newDelete = true;

                    logList.addLog("Email Cancellata con successo");
                }
                ...
                if (!action.equals("Delete"))
                    logList.addError("Non è stato possibile modificare la mail");
    ...
    private Response manageGetNewEmails(Request request) {
        ...
        if (data != null && data.getClass() == Integer.class) {
            int id = (Integer) data;    // Value of the last received mail

            /* Ottenimento di tutte le nuove mail ricevute */
